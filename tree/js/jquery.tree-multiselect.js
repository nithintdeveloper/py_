...
Tree.prototype.render = function (noCallbacks) {
  ...
  for (var ii = 0; ii < this.keysToRemove.length; ++ii) {
    ...
    //console.log(node);
    if (node) {
      selectedApis.delete(parseInt(node.getAttribute('data-value')));
      //console.log(parseInt(node.getAttribute('data-value')));
  ...
  for (var jj = 0; jj < this.keysToAdd.length; ++jj) {
   ...
   Tree.prototype.render = function (noCallbacks) {
     ...
     for (var jj = 0; jj < this.keysToAdd.length; ++jj) {
       ...
       if (inputNode) {
         //console.log(parseInt(selectedNode.getAttribute('data-value')));
         if(selectedApis.has(parseInt(selectedNode.getAttribute('data-value')))) {
             selectedApis.delete(parseInt(selectedNode.getAttribute('data-value')));
         }
         else {
             selectedApis.add(parseInt(selectedNode.getAttribute('data-value')));
         }
